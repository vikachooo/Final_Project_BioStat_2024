...
```
```{r}
choose_sheet <- function(result, year){
  if (year %in% c(2017,2024)) {result <- result[-c(1)]}
  else { 
    if (year %in% c(2018,2019,2020,2023)) {result <- result[-c(1,length(result))]}
    else {
      if (year %in% c(2021)) {result <- result[-c(1,length(result)-1, length(result))]}
      else {
        if (year %in% c(2022)) {result <- result[-c(1,16)]}
          }
        }
    }
  return(result)
...
sheet_list <- choose_sheet(sheet_list, year)
...
sheet_name = sheet_list[1]
n_skip <- 0
if (year == 2020) {n_skip <- 1} # в ексель файле 2020 год лишняя первая строка
...
filter(!(year == 2018 & hosp %in% c("b17", "b37"))) %>%
  ...
  #group_by(year, hosp) %>% summarise()
  mutate(hosp = recode(hosp, "alm_corr" = "alm", "b17_corr" = "b17",	"b37_corr"="b_37"))
```
