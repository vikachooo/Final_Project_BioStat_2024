...
```{r}
database = read_tsv("raw_database.tsv")
```
# Доделываем БД
## Отбираем колонки
```{r}
...
```{r}
database_selected %>% filter(type == 2) %>% select(starts_with('Всего_с_ОНМК')) %>%
...
database_summarized = database_selected %>% group_by(hosp, year, week, date) %>%
  summarise(across(everything(), sum, na.rm = TRUE), .groups = "drop")

database_summarized
```
```{r}
database_summarized %>% filter(hosp == "b3")
```
...
```{r}
#write_tsv(database_summarized_month, "data_summarized_by_month_Kravets.tsv")
```
...
```{r}
database_one = database_summarized_month %>% filter(hosp == "alm") %>% select(date, month)
database_one
```

```{r}
database_summarized %>% filter(year == "2021", hosp == "b3")
```
